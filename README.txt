...
It is my first ever app with React. I put up an arbitrary time limit for the dev duration: together with the research it must not be longer than two days. The solution is feature-complete, with minimal test coverage in this timeframe.
...
Remarks:
    ...
    - As a newbie to React, I would ask the architect if service workers are a thing re this task. I am not sure how much effort could or should be put into offline optimization with React. Maybe dependencies / hooks to define a cache for the referenced pictures could be a viable idea as well in a more serious application.
